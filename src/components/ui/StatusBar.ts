...
export class StatusBar extends HTMLElement {
  ...
  private setupListeners() {
    ...
    EventBus.on(
      STORE_EVENTS.VALIDATION_UPDATED,
      (
        warnings: Array<{
          message: string;
          sectionId?: string;
  ...
  render() {
    ...
    const pageCount =
      state.sections.filter((s) => s.type === 'pagebreak').length + 1;
    ...
    this.className =
      'fixed bottom-0 left-0 w-105 bg-studio-base border-t border-studio-border h-8 flex items-center px-4 justify-between z-50 select-none';
    ...
    this.innerHTML = `
      <div class="flex items-center gap-4 font-mono text-[9px] text-studio-muted h-full">
        ...
        <div class="flex gap-3 h-full items-center">
          ...
          <span id="view-toggle-status" class="text-accent-primary font-bold cursor-pointer hover:scale-110 transition-transform px-1" title="Alternar Preview (Alt+P)">VIEW: ${
            state.ui.previewVisible
              ? '<ui-icon name="eye" size="xs"></ui-icon>'
              : '<ui-icon name="eye-off" size="xs"></ui-icon>'
          }</span>
        </div>
      </div>
    ...
    this.querySelector('#view-toggle-status')?.addEventListener(
      'click',
      (e) => {
        e.stopPropagation();
        store.togglePreview();
