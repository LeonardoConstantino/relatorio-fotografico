...
export class ImageEditorModal extends HTMLElement {
  ...
  private undo() {
    ...
    ToastManager.show({ message: 'Ação desfeita', type: 'info', duration: 800 });
  }
  ...
  private setupDrawingEvents() {
    ...
    const move = (e: any) => {
      ...
      if (this.currentTool === 'brush') {
        ...
        if (this.currentTool === 'circle') this.drawCircle(this.startX, this.startY, x, y);
        if (this.currentTool === 'arrow') this.drawArrow(this.startX, this.startY, x, y);
      }
    };
  ...
  private applyDisplayZoom() {
    ...
    this.querySelector('#zoom-val')!.textContent = `${Math.round(this.currentZoom * 100)}%`;
  }
  ...
  private async save() {
    ...
    const burnMeta = (this.querySelector('#check-watermark') as HTMLInputElement)?.checked;
  ...
  render() {
    ...
    this.innerHTML = `
      <ui-modal id="modal-evidence-editor" size="lg" animation="scale">
        <h2 slot="header" class="text-white font-bold flex items-center gap-2 text-sm uppercase tracking-tight">
          <span>✎</span> Estúdio de Evidências
        </h2>

        <div class="flex flex-col gap-4">
          <!-- Toolbar Superior -->
          <div class="flex flex-wrap items-center justify-between gap-4 bg-studio-base p-2 rounded border border-studio-border">
            <div class="flex items-center gap-1.5" id="tools-group">
              <button class="btn btn-secondary !p-2 ${this.currentTool === 'brush' ? 'ring-2 ring-accent-primary' : ''}" data-tool="brush" title="Pincel">🖌️</button>
              <button class="btn btn-secondary !p-2 ${this.currentTool === 'arrow' ? 'ring-2 ring-accent-primary' : ''}" data-tool="arrow" title="Seta">↗</button>
              <button class="btn btn-secondary !p-2 ${this.currentTool === 'circle' ? 'ring-2 ring-accent-primary' : ''}" data-tool="circle" title="Círculo">⭕</button>
              <div class="w-px h-6 bg-studio-border mx-1"></div>
              <button class="btn btn-secondary !p-2" id="action-rotate" title="Girar 90°">🔄</button>
              <button class="btn btn-secondary !p-2" id="action-undo" title="Desfazer">↩️</button>
            </div>

            <div class="flex items-center gap-4">
              <select id="select-thickness" class="bg-studio-elevated text-white text-[9px] font-mono border border-studio-border rounded px-2 py-1.5 outline-none uppercase tracking-wider">
                <option value="3">Fino</option>
                <option value="6" selected>Médio</option>
                <option value="12">Grosso</option>
                <option value="24">Extra</option>
              </select>
              <div class="flex items-center gap-1.5 bg-studio-elevated p-1 rounded border border-studio-border" id="colors-group">
                <button class="w-5 h-5 rounded-full bg-[#E11D48] ${this.currentColor === '#E11D48' ? 'ring-2 ring-white' : ''}" data-color="#E11D48"></button>
                <button class="w-5 h-5 rounded-full bg-[#F59E0B] ${this.currentColor === '#F59E0B' ? 'ring-2 ring-white' : ''}" data-color="#F59E0B"></button>
                <button class="w-5 h-5 rounded-full bg-[#06B6D4] ${this.currentColor === '#06B6D4' ? 'ring-2 ring-white' : ''}" data-color="#06B6D4"></button>
              </div>
              <div class="flex items-center gap-2 bg-studio-elevated px-2 py-1 rounded border border-studio-border">
                <button id="zoom-out-editor" class="text-xs">➖</button>
                <span id="zoom-val" class="text-[10px] font-mono w-10 text-center">100%</span>
                <button id="zoom-in-editor" class="text-xs">➕</button>
              </div>
            </div>
          </div>

          <!-- Canvas Container (Centralização Corrigida) -->
          <div id="canvas-scroll-container" class="bg-black/60 rounded border border-studio-border overflow-auto h-[50vh] relative text-center">
            <div class="inline-block p-8 align-middle">
              <canvas class="cursor-crosshair shadow-2xl bg-white block"></canvas>
            </div>
          </div>

          <!-- Metadados e Marca d'água -->
          <div class="bg-studio-elevated p-4 rounded border border-studio-border space-y-4">
            ...
            <div class="flex items-center gap-3">
              <input type="checkbox" id="check-watermark" class="w-4 h-4 rounded accent-accent-primary cursor-pointer" />
              <label for="check-watermark" class="label-technical !mb-0 cursor-pointer hover:text-white transition-colors">Gravar metadados permanentemente na imagem (Marca d'água)</label>
            </div>
          </div>
        </div>
        ...
        <div slot="footer" class="flex justify-end gap-3">
          ...
          <button class="btn btn-primary" id="btn-save-final">Salvar no Relatório</button>
        </div>
      </ui-modal>
    `;
  ...
  private setupUIEvents() {
    this.querySelector('#tools-group')?.addEventListener('click', (e: any) => {
      ...
      if (tool) {
        ...
        this.querySelectorAll('[data-tool]').forEach(b => b.classList.remove('ring-2', 'ring-accent-primary'));
    ...
    this.querySelector('#colors-group')?.addEventListener('click', (e: any) => {
      ...
      if (color) {
        ...
        this.querySelectorAll('[data-color]').forEach(b => b.classList.remove('ring-2', 'ring-white'));
        e.target.classList.add('ring-2', 'ring-white');
      }
    });

    this.querySelector('#select-thickness')?.addEventListener('change', (e: any) => {
      this.currentThickness = parseInt(e.target.value);
    });

    this.querySelector('#action-rotate')?.addEventListener('click', () => this.rotate());
    this.querySelector('#action-undo')?.addEventListener('click', () => this.undo());
    
    this.querySelector('#zoom-in-editor')?.addEventListener('click', () => { this.currentZoom += 0.2; this.applyDisplayZoom(); });
    this.querySelector('#zoom-out-editor')?.addEventListener('click', () => { this.currentZoom = Math.max(0.1, this.currentZoom - 0.2); this.applyDisplayZoom(); });

    this.querySelector('#btn-save-final')?.addEventListener('click', () => this.save());
    this.querySelector('#btn-close-modal')?.addEventListener('click', () => (this.querySelector('#modal-evidence-editor') as any).close());
